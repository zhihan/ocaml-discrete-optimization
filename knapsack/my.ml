...
module Item = struct
  ...
  (* Assume x is sorted by descreasing order of unit value *)
    let rec loop (current: int) (remain:int) (sum:int) : (num*int) = 
...
module Result = struct
  ...
  let add_item (x:Item.t array) (i:int) (v:t) :t = 
    ...
    let value = num_of_int (v.lowerBound + Item.get_value x.(i)) in
    let (u, r) = Item.upper x (i+1) re in
    if (r < 0) then
      {sel=s; 
       isFull=false; 
       remain=re; 
       lowerBound = (int_of_num value);
       upperBound= (u +/ value)}
    else
      (*Solution is found *)
  ...
  let leave_item (x:Item.t array) (i:int) (v:t) :t = 
    ...
    if (r < 0) then
      ...
      {sel=s; 
       ...
       remain=0; 
       lowerBound = v.lowerBound; 
       upperBound = u +/ (num_of_int v.lowerBound)}
  ...
  let to_string v = 
    "{ sel: " ^ (Selection.to_string v.sel) ^ "; " ^
      ...
      "upperBound: " ^ (string_of_num v.upperBound) ^ " }"
end
